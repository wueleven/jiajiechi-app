<template>
  <div class="page-container">
    ...
    <div class="card">
      ...
      <div class="tip-item">6. Zepp 华米使用账号密码绑定（仅单向拉取，作为数据源）</div>
    </div>
...
import { zeppLogin, saveZeppBind, clearZeppBind, getZeppDevices, getZeppSports } from '../../services/zeppAuth.js'
...
const platforms = computed(() => [
  ...
  { key: 'zepp', name: 'Zepp 华米', letter: 'Z', iconClass: 'zepp', domain: 'api-mifit.zepp.com', ...bindData.value.zepp },
])
...
async function submitLogin() {
  ...
  try {
    ...
    } else if (currentPlatform.value === 'zepp') {
      // 技术验证: 登录 Zepp + 验证能否拉到运动数据
      const auth = await zeppLogin(username.value.trim(), password.value.trim())
      // 验证: 拉一次设备列表 / 运动列表，确认 token 有效
      let verifyMsg = ''
      try {
        const devices = await getZeppDevices(auth.appToken, auth.userId, auth.region)
        verifyMsg = ` (设备数: ${Array.isArray(devices.items) ? devices.items.length : '?'})`
      } catch (e) {
        console.warn('Zepp 拉设备失败(不影响绑定):', e)
      }
      saveZeppBind(username.value.trim(), password.value.trim(), auth)
      showToast(`Zepp 绑定成功${verifyMsg}`)
    ...
    // 技术验证期: 显示详细错误便于定位（含 Zepp 的节点/状态码）
    const detail = err && err.message ? err.message : String(err)
    showToast('绑定失败: ' + detail.slice(0, 120))
  } finally {
    loginLoading.value = false
  }
}
...
.zepp { background: linear-gradient(135deg, #7B61FF, #5B3FD6); }
